...
class HotelsDataMergerApplicationTests {
    ...
    @Test
    fun flattenJson() {
        jsonUtils.jsonFileToMap(infoS1)
            .also {
                logger.info("result {}",jsonUtils.objectMapper.writeValueAsString(it))
                assert(it[0].attributes.size == 10)
                Assertions.assertNotNull(it[0].attributes.find { it.name == "id" })
                Assertions.assertNotNull(it[0].attributes.find { it.name == "destination" })
                Assertions.assertNotNull(it[0].attributes.find { it.name == "amenities" })
            }
